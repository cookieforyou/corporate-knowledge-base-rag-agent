...
| OutputHtmlTable | bool | 否   | 是否返回HTML格式的表格内容，默认为 **False**。开启后，对于 **表格类型** 的版面块，会返回此表格的HTML格式解析内容，存放在此版面块的 `**llmResult**` 字段中。 > ⚠️ **请注意**：若开启此参数，需同步开启LlmEnhancement参数，接口响应时间也可能会增加，**请按需开启**。 | false |
...
> python3 docmind\_example\_parser.py input\_file output\_file.md
