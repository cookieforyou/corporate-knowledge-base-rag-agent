...
| B5 漏洞治理 | 11 项 CRITICAL 清零形态：五构件族版本覆盖 + kotlin-stdlib-common 声明点排除（12 章 v2.64）；全模块 699 单测绿 | ✅ 2026-08-23 机器侧（闭环判据 = 用户侧复扫，清单 B5-2） |
...
| 用户侧 7 项回传登记 | F1-F3/M1/DR1/SG1/LT1（LT1 为簇③硬前置）；用户并行执行中（2026-08-23 用户告知 LT1 同步进行） | ⏳ 待回传 |
