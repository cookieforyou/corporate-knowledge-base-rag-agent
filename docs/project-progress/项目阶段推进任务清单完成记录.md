...
> v2 重写（2026-07-31）：检索主线基于 Spring AI 2.0 模块化 RAG（方案甲+，决策见设计文档第十章 10.0）；深度解析因 ECS 2 核无 GPU 改用 DashScope 文档解析 API（决策见第九章 9.1）；v1 的 2.4 已在 Phase 1 完成故移除，2.11 并入 2.10/2.11；新增 2.16 评估最小集（Phase 5 前移）。**环境前置条件核验表见 08 章 Phase 2 节（E1-E6：ES ik 插件 ✅、gte-rerank ✅、解析 API ✅ 已决策）**。
...
| 2.2 | ParsingServiceClient 可插拔适配层 + DashScope 文档解析 API 接入（不部署本地解析服务，云 OCR 降为兜底） | kb-infrastructure | 2d | 复杂表格 PDF 输出结构化 Markdown+HTML（含表格结构） | |
