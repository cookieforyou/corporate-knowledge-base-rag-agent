...
| # | 任务 | 负责模块 | 工时估算 | 验收标准 | 完成情况 |
|---|------|---------|---------|---------|---------|
| 3.1 | 配置 MessageWindowChatMemory + RedisChatMemoryRepository + MessageChatMemoryAdvisor | kb-ai-core | 1.5d | 多轮对话上下文记忆正常 | ✅ 代码完成，待 E2E。**复审扩展**（用户拍板）：Redis 热记忆 + kb_session/kb_message PG 归档（补齐 kb_feedback 外键缺口）。**实证修正**（设计 v2.3）：① 2.0 GA Redis 仓储为 Jedis 形态（starter `spring-ai-starter-model-chat-memory-repository-redis`，前缀 `spring.ai.chat.memory.redis.*`），草图 RedisTemplate 构造器不存在；② 记忆 Advisor 缺失 CONVERSATION_ID 为 Assert 硬断言 → 独立 `agentChatClient` Bean（记忆400），评估共享 `chatClient` 保持纯 RAG、基线不受影响；③ FaultTolerantChatMemory 装饰降级（读失败→空历史、写失败→丢弃）；④ sessionId 会话协议（请求可选、同步响应回传、缺省一次性 ID）；⑤ ChatSessionService 虚拟线程异步归档（失败不扩散）。kb-ai-core 27 + kb-api 6 测试绿 |
