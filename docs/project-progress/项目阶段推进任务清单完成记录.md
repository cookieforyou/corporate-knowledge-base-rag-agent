...
| # | 任务 | 负责模块 | 工时估算 | 验收标准 | 完成情况 |
|---|------|---------|---------|---------|---------|
| 3.1 | 配置 MessageWindowChatMemory + RedisChatMemoryRepository + MessageChatMemoryAdvisor | kb-ai-core | 1.5d | 多轮对话上下文记忆正常 | ✅ 代码完成，E2E 修复 1 缺陷后待终验（nonce 测试）。**复审扩展**（用户拍板）：Redis 热记忆 + kb_session/kb_message PG 归档（补齐 kb_feedback 外键缺口）。**实证修正**（设计 v2.3）：① 2.0 GA Redis 仓储为 Jedis 形态（starter `spring-ai-starter-model-chat-memory-repository-redis`，前缀 `spring.ai.chat.memory.redis.*`），草图 RedisTemplate 构造器不存在；② 记忆 Advisor 缺失 CONVERSATION_ID 为 Assert 硬断言 → 独立 `agentChatClient` Bean（记忆400），评估共享 `chatClient` 保持纯 RAG、基线不受影响；③ FaultTolerantChatMemory 装饰降级（读失败→空历史、写失败→丢弃）；④ sessionId 会话协议（请求可选、同步响应回传、缺省一次性 ID）；⑤ ChatSessionService 虚拟线程异步归档（失败不扩散）；⑥ ECS Redis 密码适配：自动配置 jedisClient 无 password/database，以 ChatMemoryRedisClientConfig 覆盖，连接信息统一取自 spring.data.redis.*（与 Redisson 单一来源，application-infra.yml 该段因此**必须保留**）；⑦ **E2E 真跑缺陷（2026-08-05）**：RedisChatMemoryAutoConfiguration#redisChatMemory 的 @ConditionalOnMissingBean 检查含 ChatMemory 类型，用户 agentChatMemory Bean 致 Redis 仓储静默回退 InMemory（对话表面连贯、Redis 零痕迹、重启失忆、零报错）——RedisChatMemoryRepository 改显式装配 + ChatMemoryRedisWiringTest 防回归。**ECS 环境事实**：原生 Redis 8.0.5 无 Search/JSON 模块，已换 redis-stack-server（模块齐备）；容器需挂持久化卷防重建丢数据。kb-ai-core 30 + kb-api 6 测试绿 |
