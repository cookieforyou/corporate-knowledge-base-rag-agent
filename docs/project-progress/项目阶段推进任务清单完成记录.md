...
>
> **2026-08-05 状态（3.1 E2E 定案）**：3.1 全项通过——nonce 记忆命中（KB-7749 跨轮召回，检索库无此信息唯记忆能答）+ **跨会话隔离**（同问题换会话不泄露）+ PG 归档（2 会话 10 消息）+ 索引 num_docs 增长 + 持久化卷就位。E2E 清理 2 个真跑缺陷：① 自动配置条件让位致 Redis 仓储静默回退 InMemory（显式装配修复 + 防回归测试）；② RediSearch 索引仅支持 db0（REDIS_DB=0 定案，db=1 启动 fail-fast 实证）。环境事实：ECS Redis 换 redis-stack-server（原生 8.0.5 无 Search/JSON 模块）。下一步：3.9+3.10 合并项（fail-closed 安全收敛）。
...
| 3.9 | 实现 AuthAdvisor（JWT 鉴权 + 用户/租户提取） | kb-ai-core | 2d | 无 Token 请求被拒绝 | **复审合并入 3.10**：HTTP 层 SecurityConfig 已强制 JWT，Advisor 链重做鉴权系重复建设；改为「身份完整性守卫」——校验 RetrievalContext 存在且 tenantId 非空，否则 fail-closed |
| 3.10 | 实现多租户数据隔离（FilterExpression 注入 tenant_id） | kb-ai-core | 2d→1d | 跨租户数据不可见 | **复审重写**：隔离主体已在 Phase 2 落地——向量路 FilterExpression（tenant_id+is_deleted）与 BM25 路 term 过滤均经 E2E。剩余为加固：① fail-open 收敛（tenantId 缺失应拒绝而非无过滤全量可见）；② 跨租户泄露测试零覆盖，3.18 补双租户用例。RBAC 维度（doc_id/dept_id）属 3.11 增量，同一 getSecurityFilter() builder 叠加 |
