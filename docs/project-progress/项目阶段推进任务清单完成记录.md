...
| 2.16 | kb-eval 最小集：Golden Dataset（50+）+ Top-K 召回/MRR + Faithfulness，验收度量 CI 化 | kb-eval | 3d | Phase 2 验收指标全部可自动度量 | ✅ 框架完成 (2026-07-31)：EvalRunner + 检索指标（Recall/MRR/Context Precision）+ 跨厂商 LLM-as-Judge（DeepSeek 被测/qwen-max 评判）+ CI 门禁 + 负向用例集 15 条 + 标注辅助工具；golden 语料 50+ 待按 `kb-eval/src/main/resources/golden/README-标注指南.md` 标注 |
