...
| 2.5 | ES kb_chunks 索引（ik 双模式分词）+ ETL 双写 EsIndexWriter（幂等/软删同步） | kb-infrastructure/kb-etl | 2d | 入库后 ES 可检索，chunk_id 与向量库严格一致 |
...
| 2.13 | ETL 进度推送（Redis 进度键 + WebSocket 端点） | kb-etl/kb-api | 2d | 大文件不阻塞 + 前端进度条 |
| 2.14 | 前端检索调试台（各路得分/排名 + Chunk 原文，数据源 /api/v1/retrieval/search） | 前端 | 3d | 可查看每个 Chunk 的向量分/BM25分/融合分/重排分 |
