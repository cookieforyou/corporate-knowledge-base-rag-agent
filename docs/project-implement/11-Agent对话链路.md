...
> **kb_session.mode 列（Flyway V3）**：mode 是**会话级属性**（前端「切换链路即
> 新会话」——一会话恒属一链路；HITL 确认轮显式 tool 链复用同会话与归属一致）；
> 归档链 `archiveTurn` 第 4 参（Controller 同步/流式两入口 mode 均在作用域），
> `ensureSession` **首建写入首轮 mode、已存在不覆写**（不覆写由 existsById 短路
> 天然成立——链路**逐轮**事实源仍是审计表，本列为会话归属快照，两表语义不同）；
> `SessionItem` 回传 mode；前端三处——打开会话恢复 `store.mode`（合法
> rag|tool|agent 才切，续聊即走该链路；存量会话 NULL 保持当前不切 = 降级
> 语义）、会话列表条目链路徽标（知识/工具/编排三色，存量不显示）、select
> 事件携带 mode 上抛。**否决零 DDL 的消息级 metadata 方案**（语义错位：归属是
...
> Agent 在工具循环中散落吐**过渡叙述** token（「我先并行委派两个子任务…」
> 「现委派报告撰写」——开头或中途数语），几个字一吐 streamText 非空，进度行
> 永久消失而任务仍在进行（后续委派/检索进度无法显示；卡片无让位条件不受影响）。
> 修复 = 让位条件去掉 `!streamText`，进度行**常驻至流结束**（streaming 结束
> 占位区整块收起）。**三链影响分析**：agent 链修复；rag 链四锚点 PROGRESS
> 全部先于首 token（检索完成后才调 LLM），原让位语义本成立，改动后最后一条
> 「生成回答中…」常驻正文上方语义仍正确；tool 链无 PROGRESS 信号源
...
> TaskTool 委派两点（发起/终态），KnowledgeSearchTools 的
> searchKnowledge/getDocument 只 `addToolCall` **记录不推送**，子代理内
> N 次检索记录攒到委派终态快照一并带出。修复 = 两工具在记录后各补
> `emitToolCallsSnapshot()`（终态卡片随各自完成时刻逐次出现，监听器缺席
> no-op）。**不做 RUNNING 态**：① 预算闸 countExecutedSearches 按终态
> 记录计数，发起期插 RUNNING 记录会污染预算计数语义；② 检索执行期已有
> 「知识检索 n/6 次」进度行即时反馈，卡片 RUNNING 信息重复。帧量影响 =
...
> 修复 = TaskTool 委派双点各补一个 stage 进度事件（与既有 TOOL_CALL 快照
> 推送同点）：发起「委派子代理 {name}：{描述摘要 60 字}」+ 终态
> 「子代理 {name} 执行完成|执行失败」——进度行随委派自然轮转；终态为
> 完成态陈述（无进行时误导），主 Agent 综合期 token 流出进度行即隐藏
> （前端 `v-if="progressText && !streamText"` 既有让位逻辑，前端零改动）。
...
> 经 kb_message.metadata JSON 下沉（`{"traceId":…, "toolCalls":[…]}`，citations 列
> 语义保持纯溯源）；`loadMessages` 反解回传 `HistoryMessageItem.toolCalls`（readTree
> 取键，无键/解析失败降级 null 同存量数据形态）；前端 `toMessage` 映射
> `Message.toolCalls` 复用既有 ToolCallCard 渲染链路。agent 链编排委派
...
> 同题对照可判定误伤是否编排链特有。
>
> **v2.106 补注（收官注记① 改进：任务边界纪律）**：E2E 十例收官读数暴露
> 多轮会话跨任务历史污染面——编排链挂 Memory(400)（多轮互通设计特性），
> 同会话连发独立新任务时，上一任务的完整委派过程与结论随历史注入，主 Agent
> 将其误纳入当前委派计划（例7 限界上下文任务带偏重做大泥球清单、例9 E1003
> 查询带偏重做张三岗位说明；核心答案正确但委派冗余、耗时翻倍）。修复 =
> `ORCHESTRATOR_SYSTEM_PROMPT` 委派纪律前置「任务边界」段（任务理解层先于
> 委派操作层，原六条编号不动）：**每轮以当前用户消息为唯一任务来源；历史
> 轮次仅用于理解指代与延续；已完成任务不重复执行、不重复委派；独立新任务
> 不参照历史委派过程与结论**。记忆互通特性保留（指代延续价值不变）——
> 治的是「历史任务越界进入当前委派计划」而非记忆本身；契约测试钉四锚点
> 防措辞漂移（PromptTemplatesTest `orchestratorPromptPinsTaskBoundaryDiscipline`）。
> **实测无效（同日复验）**——否定式禁令措辞（「不参照历史/不重复委派」）被
> 无视，见 v2.107 补注。
>
> **v2.107 补注（收官注记① 二轮：程序式两分支 + 末尾重申）**：v2.106 部署
> 核验在场（IDEA 源码直跑，编译产物字节级确认）后复验仍全套重做旧任务。
> 归因：否定式禁令对 LLM 无着力点——模型不认为自己在「参照历史」，其心智
> 模型是「会话内任务清单累积，新消息 = 追加第二项任务」，禁令与自评不匹配
> 即被跳过（与 v2.104 治理经验一致：肯定式操作性指令——「1-3 次检索即归纳」
> 「缩小范围后重述」——遵循率远高于否定式禁令）。修复形态：**程序式两分支**
> （先判断：当前消息是否明确引用历史产物？引用 → 在历史基础上继续；未引用
> → 独立新任务，历史已交付产物一律视为已交付，不重新检索/委派/呈现）给模型
> 一个可执行判断步骤替代其「任务累积」默认心智（剥壳判据同款「先判断→按分
> 支行动」形态）；**纪律第 7 条末尾重申**钉 system prompt 结尾注意力位（「此前
> 轮次的任务均已交付完毕，与新一轮无关——除非当前消息明确引用它们」）。
...
> （确定性物理隔离，已是落地形态非设计项）。**复验实证（2026-09-07）：
> 消息层注记生效**——同会话连发两问，第二问委派只含新任务（38.4s 对照
> 被污染轮 128s），四轮迭代（否定式→程序式→末尾重申→消息层结构分隔）
> 收敛结论：**治理位置 > 措辞**。
