...
>
> **v2 修订**：第 4.2 节「智能检索子系统」表格按模块化 RAG 规范更新：并行召回、RRF 融合、重排序三行的技术实现列分别指向 `HybridDocumentRetriever`、`RrfFusion`（DocumentJoiner 语义）、`DocumentPostProcessor` 挂载的 DashScope qwen3-rerank。
>
    ...
    └─────────┬─────────┘ └────────┬─────────┘ └────────┬────────┘
              ...
              └────────────────────┼────────────────────┘
                          ...
                          └────────┬─────────┘
                                   │
                          ┌────────▼─────────┐
                          │   重排序 (Rerank)│  DashScope qwen3-rerank
                          └────────┬─────────┘
                                   │
                          ┌────────▼─────────┐
                          │   Top-K 结果     │  含各项得分和溯源信息
...
| 重排序 | Cross-Encoder 精细化排序 | DashScope qwen3-rerank API（DocumentPostProcessor 挂载） | P1 |
