...
| 功能 | 描述 | 技术实现 | 优先级 |
|------|------|---------|--------|
| 多格式上传 | PDF、Docx、PPTX、XLSX、Markdown、TXT、HTML（4.14 簇⑦ 扩容 +PPTX/XLSX，白名单纪律仅收 OOXML 新格式，旧二进制 .ppt/.xls/.doc 不收；ContentType 白名单校验，解析经 NATIVE Tika 天然兼容） | Spring MVC MultipartFile | P0 |
