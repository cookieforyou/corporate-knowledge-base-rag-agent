...
>
> [📑 返回目录](./README.md) · 最后更新：2026-09-04
>
> **v2 修订**：第 4.2 节「智能检索子系统」表格按模块化 RAG 规范更新：并行召回、RRF 融合、重排序三行的技术实现列分别指向 `HybridDocumentRetriever`、`RrfFusion`（DocumentJoiner 语义）、`DocumentPostProcessor` 挂载的 DashScope qwen3.7-text-rerank。
>
> **v2.12 修订（2026-08-08，任务 3.15）**：「Chunk 原文回看」落地形态定案——**不新建单 chunk 查询 API**，复用 `GET /documents/{docId}/chunks`（`chunksOfOwned` 已租户隔离）；SSE TRACE 的 `ChunkTrace` 投影补 `docId` 字段（帧体积纪律不变，全文不下传），前端凭 docId+chunkId 按需拉全文弹窗展示，[ref-N] 徽标与溯源条目均可点击。对话窗全功能同期落地：多轮（sessionId 前端自备 UUID、chat store 跨页保持）、rag/tool 模式显式切换、TOOL_CALL 事件 + HITL 审批卡片（approve 后自动发起携带 `approvedToolCallId` 的确认轮）、回答 markdown 渲染（marked + DOMPurify 消毒）。
>
> **v2.13 修订（2026-09-04，入口收敛定案）**：对话窗「上传文档」按钮移除——文档上传/ETL 管道入口**唯一收敛于文档管理页**。依据：两入口同调 `POST /documents/upload` 同管道，对话窗入口为功能残缺子集（单文件、无 parseRoute 显式路由选择）且属无设计依据的前端便捷添加（v2.12 对话窗功能清单本无此项）；对话窗语义为纯消费域（检索问答），知识资产入库为管理域治理操作（文档管理页具备上传/删除/重解析/替换全生命周期上下文）；对话窗上传另有「会话附件」语义误导（实际为全租户共享入库，且 ETL 异步完成前不可检索）。前端 Chat.vue 删除按钮+进度条+WS 订阅自包含块，后端零改动。
...
| 功能 | 描述 | 技术实现 | 优先级 |
|------|------|---------|--------|
| 多格式上传 | PDF、Docx、PPTX、XLSX、Markdown、TXT、HTML（4.14 Phase4簇⑦ 扩容 +PPTX/XLSX，白名单纪律仅收 OOXML 新格式，旧二进制 .ppt/.xls/.doc 不收；ContentType 白名单校验，解析经 NATIVE Tika 天然兼容） | Spring MVC MultipartFile | P0 |
| 文本密度探测 | 读取前几页计算文本密度，判断扫描件 | PDFBox 文本提取 + 启发式规则 | P1 |
| Tika 原生解析 | 电子文档直接提取文本 | `TikaDocumentReader` | P0 |
| 解析智能路由 | 扫描件/复杂表格走深度链路 | 自定义 `SmartParsingRouter`（NATIVE/DEEP/OCR 三路由，深度链路对接 DocMind 文档解析 API，云 OCR 兜底） | P1 |
    ...
    └─────────┬─────────┘ └────────┬─────────┘ └────────┬────────┘
              ...
              └────────────────────┼────────────────────┘
                          ...
                          └────────┬─────────┘
                                   │
                          ┌────────▼─────────┐
                          │   重排序 (Rerank)│  DashScope qwen3.7-text-rerank
                          └────────┬─────────┘
                                   │
                          ┌────────▼─────────┐
                          │   Top-K 结果     │  含各项得分和溯源信息
...
| 重排序 | Cross-Encoder 精细化排序 | DashScope qwen3.7-text-rerank API（DocumentPostProcessor 挂载） | P1 |
...
| Chunk 原文回看 | 点击引用查看完整 Chunk 内容 | 复用 `GET /documents/{docId}/chunks`（租户隔离）+ TRACE 投影 `docId` 通道（v2.12） | P1 |
