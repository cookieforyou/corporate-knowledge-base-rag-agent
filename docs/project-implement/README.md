...
| 章 | 文档 | 修订状态 |
|---|---|---|
| 第九章 | [知识入库 ETL 管道](./09-知识入库ETL管道.md) | v2 修订（解析路由升级、ES 双写、Contextual Retrieval 可选项）+ v2.19（簇③ D2：ES 双写 refresh → wait_for，§9.4）+ v2.21（簇④ A4：heading 路径元数据 §9.2 / Contextual 增强落地 §9.5，A/B 决策未定）+ v2.22（簇④ A4 修复批：chunk ID 确定性化 §9.3 / 语境增强有界并发 §9.5） |
