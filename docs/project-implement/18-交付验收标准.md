...
>
> [📑 返回目录](./README.md) · 最后更新：2026-08-19 · 本章为 v1 原文迁移
>
...
| 场景 | 验收线 | ECS 实测 |
|---|---|---|
| A 检索真压 | P95 < 500ms + 零失败（3 rps × 60s，链路含改写 LLM 同步调用 + 双路召回 + rerank 外部调用） | 待回填 |
| B 生成桩压 | 50 并发持续 120s 零失败（无 OOM / 线程耗尽，await DONE 全对话时延报告） | 待回填 |
| C 真实 LLM 小样本 | TTFT P95 < 2s + TPOT < 100ms（10 样本串行，计费敏感执行前确认） | 待回填 |
| D SSE 长会话 | 20 并发 180s 零中断 + 全 DONE 收口（sessionId 多轮记忆复用） | 待回填 |
