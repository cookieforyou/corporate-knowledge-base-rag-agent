...
>
> [📑 返回目录](./README.md) · 最后更新：2026-08-26 · 本章为 v1 原文迁移 + v2.59 §18.4 + v2.67 §18.4 场景 A 三路阈值（500→600）+ §18.5 GraphRAG Phase5簇④验收	口径（Phase5簇④）
>
...
| 场景 | 验收线 | ECS 实测 |
|---|---|---|
| A 检索真压 | 双路形态 P95 < 500ms / **三路融合形态 P95 < 600ms**（Phase5簇④：图路预算 ~100ms 与双路并行取 max，+100ms 余量；`LoadTestConfig.aP95ThresholdMs` 缺省同步 600）+ 零失败（3 rps × 60s，链路含改写 LLM 同步调用 + 多路召回 + rerank 外部调用） | ✅ 双路通过（2026-08-23 用户侧，缺省 3 rps 零失败；数值列待报告补录）；三路复跑待Phase5簇④启用后用户侧执行 |
