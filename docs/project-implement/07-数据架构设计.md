...
>
> [📑 返回目录](./README.md) · 最后更新：2026-08-20 · v1 原文迁移 + v2.10/v2.25-v2.28 修订（见 README 版本记录）+ **v2.54 新增 §7.6 Flyway 迁移版本化（Phase4簇⑥ 4.11）**
...
-- 1. 文档主表
-- v2.25 扩展（优化冲刺簇⑥ C1 增量重入库）：version 列 + status 枚举增 REINDEXING
-- 存量库升级 DDL：
...
CREATE TABLE kb_document (
    ...
    chunk_count     INT,
    error_message   TEXT,
    version         INT NOT NULL DEFAULT 1,        -- 优化冲刺簇⑥ C1：重入库成功 +1
...
CREATE TABLE kb_chunk (
    ...
    -- created_at：ORM 侧 @Column(updatable=false)（v2.26，优化冲刺簇⑥ C1 E2E 修复）——
...
> **v2.28 修正（2026-08-13，冲刺簇⑥ D3 集成测试实证）——pgvector 接线两则**：
...
## 7.6 Schema 迁移版本化：Flyway（v2.54 新增，Phase4簇⑥ 4.11）
...
**同源双写纪律（钉死）**：schema.sql 保留——kb-eval Testcontainers IT init script 专用（advisor 链 IT 复用，超级用户执行；Neo4j 网关 IT 独立容器不消费）。后续任何 schema 变更：① 先新增 Flyway 迁移脚本 `V(N+1)__xxx.sql`；② 同步合入 schema.sql 全量快照。表集一致性由 kb-domain `SchemaDualSourceConsistencyTest` 单测兜底（双源 CREATE TABLE 集合比对，漏同步即红灯）；列级漂移由 IT 的 ddl-auto=validate 兜底。
