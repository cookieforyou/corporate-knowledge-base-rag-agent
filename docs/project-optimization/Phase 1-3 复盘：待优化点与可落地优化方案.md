...
| ④ | 质量提升簇 | E1 → A4 → A5 | 先校准尺子（Judge 定档），再测上限杠杆（Contextual A/B），后指代消解体验；全部 eval 验证，依赖冲刺簇①扩后的数据集 | 4-5d | kb-eval A/B + 追问 E2E |
...
**顺序理由**：① 零架构风险、最快见效，直击低分例，且练出后续各簇共用的 eval A/B 流程肌肉；② 是最大风险缺口（间接注入已现网化 + 消毒盲区），域独立紧随其后；③ 小件批量清仓；④ 依赖冲刺簇①的数据集与 A/B 经验；⑤⑥ 大件压轴——C1 为 Phase 4.4-4.6 铺路，D3 到达定案触发时点。
...
| ③ 工程健壮性小件批 | ✅ 完成（D1+D2 全收口） | 2026-08-11 | D1：源码核验 deepseek starter 请求 record 无 stream_options 字段（2.0.0 字节码）→ 主模型 `deepSeekChatModel` 改手工装配 OpenAI 兼容形态开 include_usage（端点支持经官方文档核验），备用模型同步开启；**实证修正**：同名用户 Bean 让位自动装配方案被 ApplicationContextRunner 实证否决（Boot 4.1 抛 BeanDefinitionOverrideException），改经 starter 类级 `@ConditionalOnProperty(spring.ai.model.chat)` 门控置 none 让位，DeepSeekModelOverrideWiringTest 双向钉死；TokenBudgetAdvisor/审计末块回写零改动自愈（流式配额漏算 + 审计 token 列 null 双修复）；maxTokens wire 字段 max_tokens 经字节码核验与 starter 时代一致。D2 四件：① rerank RestClient connect/read 超时（rag.rerank.timeout-seconds 默认 5s，超时走 fusion_score 截断降级）；② HybridDocumentRetriever 执行器每请求 new → 共享 Bean hybridRetrievalExecutor；③ EsIndexWriter refresh(true) → wait_for（级联删除路径不变）；④ EMPTY_CONTEXT_PROMPT 无参渲染回归钉死零占位符约束。报告 D2 第 4 项（批次配置化）已被冲刺簇① A3 吸收。设计回写 9/10/11/12 章 v2.19 + README；全 reactor 单测绿。E2E 后补（2026-08-12 通过）：启动验证 + 流式对话后 Redis 账本累加、kb_audit_log token 列非空核验 |
| ④ 质量提升簇 | ✅ 完成（E1 全闭环 / A4 A/B 定案 Contextual 默认开 / A5 E2E 通过） | 2026-08-13 | E1：Faithfulness 门禁容忍策略（噪声带 0.05 WARN 不 FAIL + 分类均值地板 3.5「单维不崩」）+ EVAL_RUN_LABEL 复跑快照 + EVAL_JUDGE_AGREEMENT_SAMPLE 分层抽样一致率打分表 + 报告分类分解（16 章 v2.20）。**定档复跑（2026-08-12）**：thinking 开/关漂移 F +0.025（4.163→4.188）在噪声带内、FACTOID/REASONING 逐位重合 → 定档 thinking 关（现默认形态）；门禁维持阈值 4.0（余量 0.163）/地板 3.5；**一致率人工填 20 条与 Judge 全一致 = 100% ≥ 85% → E1 全闭环**。A4：HtmlProtectingSplitter 六级标题栈 heading_path 落三存储面（§9.2 v2.21）+ ContextualEnrichmentTransformer 复活（默认关，§9.5 v2.21）。**a4-heading-only 复跑暴露度量尺断裂并当日修复**：重入库令随机 UUID chunk ID 换代 → Golden expectedChunkIds 整体失配、检索三指标全 0.000（生成侧 F 反涨 4.750 证检索本身正常）。修复批（9.3/16 章 v2.22/v2.21）：① chunk ID 确定性化 nameUUID（文档名#序号#增强前原文，重入库/A-B 两臂复现）；② 文档级兜底检索指标（expectedDocs×file_name，跨重入库恒稳定，无门禁仅观测）；③ `--eval.annotate-all` 全量重标注通道；④ 语境增强串行 LLM 改虚拟线程有界并发（默认 8，治理大文档分钟级阻塞）。A5：CompressionQueryTransformer 历史感知改写（10 章 v2.21；**追问 E2E 通过 2026-08-13，冲刺簇④ 全收口**）。**重标注完成（2026-08-12）**：删库重传（确定性 ID + heading_path 生效）→ annotate-all + 全库 chunk 快照圈定——口径：全库 ground truth（不限 Top-8，召回度量须独立于检索结果）+ 开放枚举题代表性锚点 + 拆分表圈全（dm-13 为真实难点）；102 条迁移（80 正向 110 chunk 引用全部 UUID v3 / 22 负向），GoldenDatasetLoaderTest 5 例回归钉死（16 章 v2.21 注 5）。**A/B 定案（2026-08-12）**：双臂全量重入库 chain 探针——**Recall@5 0.902→0.931 / MRR 0.888→0.933 / CP 0.851→0.886**，Doc 级同向；靶点 dm-13 拆分表 0.000→0.667（语境前缀 = 表格稀薄语义的主题信号）、dm-02 0.50→1.00，cross 9 例 7 改善；生成侧中性（F 4.813→4.725 噪声带内，TABLE 分类 4.267→3.800 列观察项）→ `kb.etl.contextual.enabled` 默认转 true（9.5 v2.23）。语料审计：两臂 chunk 快照全量比对——168 ID 逐位复现（确定性 ID 契约实证）/ heading_path 零漂移 / 增强 166 跳过 10 均设计行为。附带 cross-08 标注补漏（质保期表 chunk 属「持续时长」证据，口径②细化：全库证据 ≤K 圈全，16 章 v2.21 注 6）。残留：cross-08 两臂均 0（抽象聚合查询语义鸿沟，查询侧难点）。累计新增 39 例单测，全 reactor 绿 |
| ⑤ 护栏可观测与注入门禁 | ✅ 完成（B2 全收口：S3+S6） | 2026-08-13 | S3（commit `967ef39`）：AiBusinessMetrics 5 项 `rag.guardrail.*` 分列计数器（injection.blocked / pii.masked / output.replaced / rate.limited / token.budget，延续零标签惯例）接四护栏调用点；拒绝型审计行复用 AuditTraceAdvisor REJECTED 三态既有通道零改动；非拒绝型干预（PII 掩码/输出替换）计指标，审计行维持 SUCCESS 不加 error_code 标记（维持「SUCCESS→null」不变量免 schema 变更）；「原文摘要哈希」简化不做（审计行已含租户/攻击类型/脱敏原文）。S6：kb-eval INJECTION 分类 + `injection-qa.json` 四类 44 条（DIRECT/ENCODING_BYPASS/JAILBREAK/MULTILINGUAL 各 11，Golden 102→146）+ evalGuardrailChatClient 确定性判定（仅 InputSanitizeAdvisor，捕获 PROMPT_INJECTION → BLOCKED，零 Judge 零检索）；**门禁口径定案**：拦截率 ≥95% 仅对 L1 机制防域子集（DIRECT+ENCODING_BYPASS）门禁——L1 关键词匹配不拦语义化越狱/非中英载荷属设计行为，全量门禁必然首轮红；JAILBREAK/MULTILINGUAL 观察集逐类报告喂 S5/S8；间接注入移交 Phase 5/S5（毒化语料 + Judge 判定「是否执行注入指令」，与 Noise Robustness 同族，口径提案落 12 章 §12.6）。样本与词表防域编程式自洽校验（GoldenDatasetLoaderTest：门禁子集归一后必命中/编码样本归一前必不命中/观察集必不命中），期间修复 1 条样本设计缺陷（零宽空格词间粘合）。报告「── 安全性（注入拦截）──」三级读数。设计回写 12 章 v2.24（G4 闭环 + §12.4.3 销项 + §12.6）+ 16 章 v2.24 + README v2.24；新增 13 例单测，全 reactor 绿 |
| ⑥ 文档生命周期与回归保险 | 未开始 | — | — |

> **弹性条款**：推进中若评估暴露新事实（如冲刺簇①扩展无显著收益、冲刺簇④ Contextual A/B 不达标），簇构成与顺序可随时调整，调整及理由记入进度表。
