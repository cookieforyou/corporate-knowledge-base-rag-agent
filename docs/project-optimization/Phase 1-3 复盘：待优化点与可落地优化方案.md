...
| ④ 质量提升簇 | 🔄 E1 定档完成，一致率填分/A-B/E2E 待执行 | 2026-08-12 | E1：Faithfulness 门禁容忍策略（噪声带 0.05 WARN 不 FAIL + 分类均值地板 3.5「单维不崩」）+ EVAL_RUN_LABEL 复跑快照 + EVAL_JUDGE_AGREEMENT_SAMPLE 分层抽样一致率打分表 + 报告分类分解（16 章 v2.20）。**定档复跑（2026-08-12）**：thinking 开/关漂移 F +0.025（4.163→4.188）在噪声带内、FACTOID/REASONING 逐位重合 → 定档 thinking 关（现默认形态）；门禁维持阈值 4.0（余量 0.163）/地板 3.5；A5 入链后 chain 基线 R 0.904 / MRR 0.897 / CP 0.847 / F 4.163（检索三指标 ≥ 旧基线全线上涨）为 A4 重入库冻结快照；余一致率人工填分 20 条。A4：HtmlProtectingSplitter 六级标题栈 heading_path 落三存储面（@Transient 载体免 ALTER，§9.2 v2.21）+ ContextualEnrichmentTransformer 复活（消毒后语境前缀，kb-etl 引 spring-ai-openai 手工装配经济模型，默认关，§9.5 v2.21；重入库窗口 + on/off 双快照 A/B 待 E1 定档后执行）。A5：源码核验 RewriteQueryTransformer 不消费历史 → 切 CompressionQueryTransformer（Query.history() 显式消解 + 中文 Prompt 硬契约，与 440 预写零冲突，10 章 v2.21；追问用例 E2E 待执行）。三项共新增 23 例单测，全 reactor 绿 |
| ⑤ 护栏可观测与注入门禁 | 未开始 | — | — |
| ⑥ 文档生命周期与回归保险 | 未开始 | — | — |
