...
| ④ 质量提升簇 | 🔄 实现批完成，定档/A-B/E2E 待执行 | 2026-08-12 | E1：Faithfulness 门禁容忍策略（噪声带 0.05 WARN 不 FAIL + 分类均值地板 3.5「单维不崩」）+ EVAL_RUN_LABEL 复跑快照 + EVAL_JUDGE_AGREEMENT_SAMPLE 分层抽样一致率打分表 + 报告分类分解（16 章 v2.20；thinking 开/关定档复跑与人工填分待执行）。A4：HtmlProtectingSplitter 六级标题栈 heading_path 落三存储面（@Transient 载体免 ALTER，§9.2 v2.21）+ ContextualEnrichmentTransformer 复活（消毒后语境前缀，kb-etl 引 spring-ai-openai 手工装配经济模型，默认关，§9.5 v2.21；重入库窗口 + on/off 双快照 A/B 待 E1 定档后执行）。A5：源码核验 RewriteQueryTransformer 不消费历史 → 切 CompressionQueryTransformer（Query.history() 显式消解 + 中文 Prompt 硬契约，与 440 预写零冲突，10 章 v2.21；追问用例 E2E 待执行）。三项共新增 23 例单测，全 reactor 绿 |
| ⑤ 护栏可观测与注入门禁 | 未开始 | — | — |
| ⑥ 文档生命周期与回归保险 | 未开始 | — | — |
