...
**当前阶段**：Phase 1-3 与优化冲刺完成；**安全加固专项：簇①②③④收官、簇⑤ E2E 通过、簇⑥ F1/F2 E2E 通过，G1/G2 机器侧就绪（tools/redteam/ Promptfoo red-team 通道 + probe_corpus_coverage.py 覆盖复算探针——基线 47/13/9/58 静默区锚点；G1 首跑/G2 带外扩词待用户侧，eval L2 复跑簇⑥收官后；F2 前端第五 Tab 已落地——词表运营形态再评估定案维持 Git Ops，Plan C 复审触发条件钉死，12 章 v2.52）**（B5 基线入档治理另跟踪；簇④ 抑制率首跑基线 1.000 入档，探针口径缺口登记转下冲刺；载荷纪律沿簇①零字面词形态）。遗留口径 3.11/5.4 缓做、3.16 取消、12.4 S1-S8 各簇消化完毕（S9 不排期）。设计依据 `docs/project-implement/README.md`；**过程细节与 E2E 在** `docs/project-progress/项目阶段推进任务清单完成记录.md`（按任务行定位，勿整读）。
...
- **词表工程（簇① v2.43）**：词项模型（value 逐条编码加载层解码）+双源合并（结构化∪CSV，外部缺失回落缺省）；REGEX 模式轨（领域裸词不入 BLOCK）；带外导入 import_words.py/import_corpus.py（AI 零接触词面）；**FLAG 观察**：命中只计数+审计标记——`rag.guardrail.flagged`（side/family 标签）+审计 `guardrail_flags` 列（ECS 先 ALTER）；新增词默认 FLAG，零误伤确认方转 BLOCK；输出三分类话术+系统提示金丝雀；语料 base64+指纹锚点；零字面载荷；见 §12.7。**热重载（簇⑥ F1 v2.49，S8 销项）**：`GuardrailRulesRegistry`（kb-commons）全上下文单一词表持有——注入/输出双 volatile 快照，`reload()` 原子替换+监听器推送、失败 fail-keep 保旧；`GuardrailReloadCoordinator`（kb-ai-core）双触发——pub/sub 频道 `rag:guardrail:reload` 即时信号 + file: 源 mtime 轮询回落（classpath 源不轮询），`@ConditionalOnWebApplication` 结构隔离 kb-eval（测量一致性）+`rag.guardrail.reload.enabled` 阀门；五消费方（InputSanitize/SemanticInjection/IndirectScan/OutputGuardrail/SanitizingTransformer）Observer 化——volatile 承接推送调用侧零改动，构造期装载退役；指标 `rag.guardrail.reload.succeeded/failed`；信号发布 tools/guardrail/publish_reload_signal.py；**F2 kb-admin 端点**：GET /admin/guardrail/rules（六条件过滤活视图）+ POST /drill 命中演练（运行时同口径，不计指标不落审计），视图元数据形态 value 明文不回显（指纹+长度）
