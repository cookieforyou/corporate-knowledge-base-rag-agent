...
- 启动：本地开发 fat jar 直起（坑位㉘）；**生产 ECS 容器化形态（v2.55 簇⑥）**：根 Dockerfile + `infra/docker-compose.app.yml`（禁 latest / healthcheck）+ `infra/.env.example` Secrets 模板 + kb-stack.service 全栈自启，见 17 章 §17.4；实操手册 docs/delivery/；前端开发 `npm run dev`，生产 = 宿主 nginx 同源反代（`infra/nginx/frontend.conf`）。
