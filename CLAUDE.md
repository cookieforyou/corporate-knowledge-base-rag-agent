...
**当前阶段**：Phase 1 完成；Phase 2 收尾（2026-08-04，Golden 74 条基线 Recall@5 0.971 / MRR 0.910 / Faithfulness 4.093 / Negative Rejection 1.00）；**Phase 3 进行中（已完成 16 项）**：3.1 记忆、3.2 主备路由、3.3/3.4 工具+HITL、3.5-3.10 护栏/配额/租户隔离、3.12 审计（3.14 随之达成）、3.13 指标、3.15 对话窗、3.19 双链路拆分 + kb-ai-agent 独立、5.4 收窄版意图路由提前落地；3.11 缓做、3.16 取消（Casdoor 统一管理）；护栏加固立项不排期（12.4 S1-S9）。设计唯一依据 `docs/project-implement/README.md`（v2 + v2.1-v2.13 修正）；**过程细节与 E2E 记录全部在** `docs/project-progress/项目阶段推进任务清单完成记录.md`（按任务行定位，勿整读）。
...
> **版本说明**：PG 18/ES 9.4.2/Milvus 2.6/Redis 8 指 ECS 服务端版本；pom 客户端库版本（如 elasticsearch-java 8.14.3）独立管理。
...
- **API 端口 8090**：8080 被占，经 `SERVER_PORT=8090` 启动变量配置；前端 `frontend/.env` 的 `BACKEND_URL=http://localhost:8090` 配套（Vite 代理 `/api`）。
...
**双链路架构（3.19）**：`ragAgentChatClient`（kb-ai-core，纯检索零工具，链 10/30/100/110/300/400/440/450/500）+ `toolAgentChatClient`（kb-ai-agent，纯工具零检索，链 10/30/100/110/300/400/1000 + defaultTools）；请求体 `mode: rag|tool` 显式分流（缺省 rag，非法值 400 INVALID_MODE，跨链自动路由留 5.4）；共享 smartRoutingChatModel / agentChatMemory（跨链互通）/ 护栏配额 Advisor / RetrievalContext；toolContext 仅 ToolChatService 组装（RagChatService 签名物理消除 HITL 凭证）；kb-eval 注入独立 chatClient 零影响
...
**业务指标（3.13）**：`metrics/AiBusinessMetrics` 统一注册中心——rag.feedback.like/dislike（3.17 接线点）、rag.retrieval.total/hit/latency、rag.tool.call.total/success/pending（ToolCall.status 分桶）、rag.token.total/budget.rejected、rag.routing.chitchat/knowledge；不带租户标签防基数膨胀；SecurityConfig 放行 prometheus/metrics/** 端点
...
- SSE 协议：`/chat/stream` 无名 TOKEN/ERROR/`[DONE]` + 命名 TRACE（三路溯源，final 与 [ref-N] 对齐；ChunkTrace 含 docId——「查看原文」通道）/ TOOL_CALL（仅 tool 链）
- 前端对话窗（3.15）：chat store 自备 sessionId 多轮 + rag/tool 切换；TOOL_CALL→审批卡片（approve 后自动发携带 approvedToolCallId 的确认轮）；[ref-N]/溯源条目点击经 docId 弹原文（复用租户隔离 chunks API）；marked+DOMPurify 渲染管线
...
- 评估（kb-eval）：双探针 `eval.probe`=auto/vector/hybrid；`chatClient` Bean 独立注入，被测链路切换零感知；Golden 74 条；提速三件套 `eval.concurrency`(5) / `eval.retrieval-only` / Milvus 预热；报告 stdout + `target/eval-report.txt`
...
- 测试：六模块 211 单测绿（core 132/agent 23/eval 12/etl 14/infra 10/api 20）
...
- Phase 2 检索架构为 Spring AI 2.0 模块化 RAG（组件形态见「检索与对话链路」主链路条）；Milvus 原生混合检索经源码级核验后否决，决策全文见 `docs/project-implement/10-混合检索引擎.md` §10.0
