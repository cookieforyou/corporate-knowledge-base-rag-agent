...
- 启动：本地开发 fat jar 直起（坑位㉘）；**生产 ECS 容器化形态**：根 Dockerfile + `infra/docker-compose.yml`（全栈 app+监控）+ `.env.example` Secrets 模板 + kb-stack.service 自启，见 17 章；手册 docs/delivery/；前端生产 = 宿主 nginx 同源反代。
...
**面板与统计（Phase4簇②）**：Grafana 五面板（含 supplier-sla）+ 监控 compose 生产形态 + 告警 14 条自检矩阵；统计 API GET /api/v1/stats/overview|documents/processing（租户守卫）
...
**A2A 协议端点（Phase5簇⑥ 批2）**：`rag.a2a.enabled` 缺省关（Controller 条件装配 404 缺位）；a2a-java SDK Quarkus/CDI 绑定判负 → **自研协议层（零新增依赖）**：v1.0 JSON-RPC `SendMessage` 同步应答 + Agent Card 发现（`/.well-known/agent-card.json`）；JWT 认证（/mcp 同款 matcher）+ 身份守卫 fail-closed；复用 `chatRag`（会话域 `a2a-{去横线 contextId}` 钉 36 字符，坑㊿）；错误码 -32009/-32601/-32602/-32603；指标 `rag.a2a.*`

**平台层加固（安全簇②）**：CORS 白名单（allowCredentials 显式 false）；上传 50/60MB + chat body 1MB 超限 413；CSP/frameOptions/HSTS 显式钉；actuator include 白名单钉死；dependency-check+CycloneDX（**NVD API key 强制**）；台账 12 §12.9 / 17 §17.3
...
- **实证坑**（钩子列表；全量台账 ①-㊿ 根因与治法见 19 章附录 E）：② 空证据拒答须 allowEmptyContext=false+模板；⑦ ChatMemory 自动配置让位须显式装配；⑬ Boot 4.1 迁 Jackson 3 注入 tools.jackson JsonMapper；⑭ 跨厂商转发前以备用 options 重建 Prompt；⑮ qwen 商业版须 enable_thinking=false；㉗ 流式 trace：builder 单参=NOOP registry；ThreadLocal 不跨线程走 contextWrite；㉘ spring-boot:run 跑兄弟模块旧 jar 须先 install；㉙ @Query 可选参数 PG 预编译雷，统一 Specification；㉚ MCP 须显式钉 protocol: STREAMABLE；㉜ dependency-check 13.x NVD key 强制；㊱ Bolt 禁 nginx http 块反代，须 stream 块 L4；㊴ bolt+s 长闲置连接须出借前探活；㊶ Neo4j 驱动不容中断，超时非打断式 cancel(false)；㊸ 配置缺省被 yml 显式段遮蔽，Java+yml 双源同查；㊹ yml 插段核父级链缩进，「配置写了」≠「生效」；㊺ 容器内同类型 Bean 注入歧义，显式 @Qualifier，开启态完整启动是独立验证面；㊾ 钉钉 SDK 监听器须匿名类（lambda 泛型签名拒绝）；㊿ 会话域前缀标识拼装核对 kb_audit_log.session_id VARCHAR(36)——前缀 ≤4 字符 + 去横线 UUID 32 = 36 恰好（mcp-/a2a- 两族已钉满）。另：Spring 7 MockHttpServletRequest 移除 setContentLength 用 setContent
...
11. **簇 tag 命名规约**：簇编号 tag 必须带阶段前缀且紧凑无空格——`Phase4簇N / Phase5簇N / 安全簇N / 冲刺簇N`；新增产出禁用裸「簇N」
