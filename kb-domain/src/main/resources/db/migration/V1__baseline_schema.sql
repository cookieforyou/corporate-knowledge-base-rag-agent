...
--
-- 【同源双写纪律，钉死】后续任何 schema 变更：
--   ① 先新增 Flyway 迁移脚本 V(N+1)__xxx.sql（本目录）；
--   ② 同步将变更合入 schema.sql 全量快照（kb-eval Testcontainers init
--      script 专用，见第 15 章）。两源表集一致性由
--      SchemaDualSourceConsistencyTest 单测兜底。
--
-- 【baseline 语义】spring.flyway.baseline-on-migrate=true：
--   - 存量库（ECS 现网，表已全在、无 flyway_schema_history）：首跑插
--     baseline 行（version=0）后应用本脚本——IF NOT EXISTS 全 no-op，
--     仅登记 flyway_schema_history 两行，零结构变更；
--   - 空库（全新环境）：直接应用本脚本建全套 schema。
--   CREATE EXTENSION IF NOT EXISTS 对已装扩展短路返回（NOTICE），先于
--   权限检查（PG 源码 extension.c 存在性检查在前）——非 superuser 的
--   应用连接用户在存量库上执行安全。
-- ============================================================
