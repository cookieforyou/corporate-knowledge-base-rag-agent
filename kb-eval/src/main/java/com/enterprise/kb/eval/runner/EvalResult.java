...
public record EvalResult(
    ...
    double docRecall,           // 文档级兜底（冲刺簇④ A4 修复，16 章 v2.21）
    double docMrr,
    ...
    String judgeReason,
    String injectionVerdict,    // 仅 INJECTION 用例（冲刺簇⑤ B2 S6）：BLOCKED / NOT_BLOCKED（L1 链）
    String l2InjectionVerdict,  // 仅 INJECTION 用例且 eval.guardrail.l2-enabled（安全簇⑤ E2）：
                                // L1+L2 联合链判定 BLOCKED / NOT_BLOCKED；关闭时 null
    String l2RawVerdict,        // L2 二判原始裁决（Phase5簇② 批5 路径 a，度量盲区清偿）：
    ...
    // ── Phase 5 扩展指标（Phase5簇② 5.8，16 章 §16.2；接线落地 16 章 v2.82：AC/CA/HR 门禁，NRob 观察）──
    ...
    String noiseAnswer          // Noise Robustness 答案 B（混噪评估侧生成，Phase5簇② 批2）：
                                // 人审 NRob 需对照 A/B 两答案——校准表材料；未抽中 → null
) {
    /** INJECTION 判定：护栏抛 PROMPT_INJECTION 被捕获（冲刺簇⑤ B2 S6） */
