...
│   └── JudgeModelConfig.java         # judgeChatClient Bean（百炼 qwen3.8-flash，跨厂商评判）
├── dataset/
...
| `DASHSCOPE_API_KEY` | Judge 模型（qwen3.8-flash） |
...
| `eval.judge.model` | qwen3.8-flash | `EVAL_JUDGE_MODEL` 覆盖（更好效果可换更高档模型） |
...
- **调用量**：每条正向用例 = 1 次 DeepSeek（被测）+ 2 次 qwen3.8-flash（Judge）；负向用例 = 1 + 1。50 条全量约 135 次模型调用，qwen3.8-flash 成本可忽略；CI 高频触发时用 `sample-size` 抽样。
