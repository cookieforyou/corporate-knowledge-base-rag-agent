 ...
 * @ConditionalOnMissingBean 检查 {RedisChatMemoryRepository, ChatMemory,
 * ChatMemoryRepository}，用户 ChatMemory Bean 先注册导致 Redis 仓储静默让位、
 * 回退 InMemory——多轮对话表面连贯（进程内记忆）但 Redis 零痕迹、重启失忆。
 *
...
class ChatMemoryRedisWiringTest {
    ...
    private final ApplicationContextRunner contextRunner = new ApplicationContextRunner()
        ...
        .withPropertyValues(
            ...
            "spring.ai.chat.memory.repository.redis.index-name=kb-chat-memory-idx",
            "spring.ai.chat.memory.repository.redis.key-prefix=kb:chat-memory:");
