...
public class RetrievalContext {
    ...
    /**
     * 进度监听器（Phase5簇⑥ 体验批3）：Controller 流式入口注册（Sinks 合流通道），各信号源
    ...
    /**
     * 进度事件（Phase5簇⑥ 体验批3，kb-api Controller 转换为 SSE PROGRESS / TOOL_CALL 帧）。
     *
    ...
    /**
     * 委派终态原地回写（Phase5簇⑥ 体验批3）：定位最后一条同名 RUNNING 记录替换为终态
    ...
    public record ToolCall(String toolName, String status, String approvalId, String summary) {
        ...
        /** 执行失败态（Phase5簇⑤ 5.3：TaskTool 委派超时/异常——错误文本回流主 Agent，不击穿主链） */
        public static final String STATUS_FAILED = "FAILED";

        /**
         * 进行中态（Phase5簇⑥ 体验批3，TOOL_CALL 帧实时化）：委派发起时写入（预算闸
