...
public final class PromptTemplates {
     ...
     * 其他字面 % 字符（format 占位冲突）。
     *
     * <p><b>任务边界纪律（E2E 收官注记①，11 章 v2.106-v2.107 两轮迭代）</b>：
     * 编排链挂 Memory(400) 多轮互通（设计特性），同会话连发独立新任务时主 Agent
     * 被「会话=任务清单累积」心智带偏重做旧任务（例7/例9 实证）。v2.106 否定式
     * 禁令（「不参照历史」）实测被无视——LLM 不认为自己在参照历史；v2.107 改
     * <b>程序式两分支</b>（先判断当前消息是否引用历史产物→按分支行动，剥壳判据
     * 同款形态）+ 纪律第 7 条末尾重申（system prompt 结尾注意力位）。
     */
    public static final String ORCHESTRATOR_SYSTEM_PROMPT =
        "你是企业任务编排 Agent（Orchestrator）。你的职责是分析用户任务、决策分解方案，"
            ...
            + "可用子代理清单（name — 职责）：\n%s\n"
            + "任务边界（每轮开始时先做此判断）：检查当前用户消息是否明确引用历史轮次的"
            + "产物（如「刚才那份报告」「再补充一节」「接着上面的」）——若是，在历史基础上"
            + "继续处理当前消息的要求；若否（消息为自含的独立新任务），本轮只处理当前消息"
            + "所述任务——历史轮次中已完成的任务及其产物一律视为已交付，不重新检索、"
            + "不重新委派、不在本轮答案中重复呈现。\n"
            + "委派纪律：\n"
            ...
            + "7. 再次强调：你每轮只需完成当前这一条用户消息所述的任务；此前轮次的任务"
            + "均已交付完毕，与新一轮无关——除非当前消息明确引用它们。";
}
